...
Finally, while `benvo`'s are useful for making the construction and manipulation of built environment data easier, as shown above, they also facilitate the use of this relational data for model fitting via, for example the [`rsstap` package](http://apeterson91.github.io/rsstap), which we can use to estimate the association of restaurant proximity to schools with the probability of obesity at those schools.
